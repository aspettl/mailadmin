...
class AccountTest < ActiveSupport::TestCase
  test "should not validate empty password (none set)" do
    account = Account.new
    assert_not account.matches_crypted_password?('')
  end

  test "should not validate empty password (empty string set instead of crypt)" do
    account = Account.new(crypt: '')
    assert_not account.matches_crypted_password?('')
  end

  test "should not validate empty password" do
    account = Account.new(crypt: ''.crypt('aa'))
    assert_not account.matches_crypted_password?('')
  end

  test "should validate DES passwords" do
    account = Account.new(crypt: 'test'.crypt('aa'))
    assert account.matches_crypted_password?('test')
  end

  test "should validate MD5 passwords" do
    account = Account.new(crypt: 'test'.crypt('$1$aaaabbbb'))
    assert account.matches_crypted_password?('test')
  end

  test "should validate SHA-256 passwords" do
    account = Account.new(crypt: 'test'.crypt('$5$aaaabbbbccccdddd'))
    assert account.matches_crypted_password?('test')
  end

  test "should validate SHA-512 passwords" do
    account = Account.new(crypt: 'test'.crypt('$6$aaaabbbbccccdddd'))
    assert account.matches_crypted_password?('test')
  end

  test "should validate bcrypt passwords" do
